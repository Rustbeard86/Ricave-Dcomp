...
public class SSAOPro : MonoBehaviour
{
    ...
    private void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        if (this.Blur != SSAOPro.BlurMode.None)
        {
            ...
            RenderTexture renderTexture = RenderTexturePool.Get(source.width / num2, source.height / num2, 0, RenderTextureFormat.ARGB32);
            RenderTexture renderTexture2 = RenderTexturePool.Get(source.width / this.Downsampling, source.height / this.Downsampling, 0, RenderTextureFormat.ARGB32);
            Graphics.Blit(renderTexture, renderTexture, this.Material, 0);
            Graphics.Blit(source, renderTexture, this.Material, num);
            ...
            for (int i = 0; i < this.BlurPasses; i++)
            {
                ...
                Graphics.Blit(renderTexture, renderTexture2, this.Material, (int)pass);
                renderTexture.DiscardContents();
                this.Material.SetVector("_Direction", new Vector2(0f, 1f / (float)source.height));
                Graphics.Blit(renderTexture2, renderTexture, this.Material, (int)pass);
                renderTexture2.DiscardContents();
            }
            if (!this.DebugAO)
            {
                this.Material.SetTexture("_SSAOTex", renderTexture);
        ...
        RenderTexture renderTexture3 = RenderTexturePool.Get(source.width / this.Downsampling, source.height / this.Downsampling, 0, RenderTextureFormat.ARGB32);
        Graphics.Blit(renderTexture3, renderTexture3, this.Material, 0);
        if (this.DebugAO)
        {
            Graphics.Blit(source, renderTexture3, this.Material, num);
            Graphics.Blit(renderTexture3, destination);
            RenderTexturePool.Return(renderTexture3);
            return;
        }
        Graphics.Blit(source, renderTexture3, this.Material, num);
        this.Material.SetTexture("_SSAOTex", renderTexture3);
        Graphics.Blit(source, destination, this.Material, 7);
        RenderTexturePool.Return(renderTexture3);
    }
