...
{
    public class UsePrompt_Anvil : UsePrompt_WheelSelector
    {
        protected override IEnumerable<WheelSelector.Option> GetOptions(Action_Use intendedAction)
        {
            ...
            {
                if (item.Spec.Item.IsEquippable && (item.Spec.Item.ItemSlot == Get.ItemSlot_Weapon || item.Spec.Item.ItemSlot == Get.ItemSlot_Armor) && RampUpUtility.AffectedByRampUp(item))
                {
                    Item localItem = item;
