...
{
    public class UsePrompt_EnchantingTable : UsePrompt_WheelSelector
    {
        protected override IEnumerable<WheelSelector.Option> GetOptions(Action_Use intendedAction)
        {
            ...
            {
                if (item.Spec.Item.IsEquippable && (item.Spec.Item.ItemSlot == Get.ItemSlot_Weapon || item.Spec.Item.ItemSlot == Get.ItemSlot_Armor))
                {
                    Item localItem = item;
