...
{
    public class UseEffect_Visual_FlintlockPistolMiss : UseEffect
    {
        protected UseEffect_Visual_FlintlockPistolMiss()
        {
        }

        public UseEffect_Visual_FlintlockPistolMiss(UseEffectSpec spec)
        ...
        public override IEnumerable<Instruction> MakeUseInstructions(Actor user, IUsable usable, Target target, Target originalTarget, BodyPart targetBodyPart)
        {
            if (user != null)
            {
                ...
                yield return new Instruction_Sound(Get.Sound_GunMiss, new Vector3?(user.Position), 1f, 1f);
